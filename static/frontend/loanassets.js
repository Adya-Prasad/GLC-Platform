...
export async function renderLoanAssets() {
    ...
    for (const a of apps) {
        ...
        const docsHtml = (docs && docs.length > 0)
            ? docs.map(d => `
                <div class="flex items-center justify-between py-1 border-b border-color-gray-400">
            ...
            `).join('')
            : `<div class="py-4 text-gray-500">No documents uploaded/found for this loan.</div>`;
